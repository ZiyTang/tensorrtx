#ifndef _RADIAN_LAYER_H
#define _RADIAN_LAYER_H
...
namespace Radian
{
    ...
    struct alignas(float) DetectionWithRad {
...
namespace nvinfer1
{
    class API RadianLayerPlugin : public IPluginV2IOExt
    {
    public:
        RadianLayerPlugin(int classCount, int netWidth, int netHeight, int maxOut, const std::vector<Radian::YoloKernel>& vYoloKernel);
        RadianLayerPlugin(const void* data, size_t length);
        ~RadianLayerPlugin();
    ...
    private:
        ...
        int mMaxOutObject;
        std::vector<Radian::YoloKernel> mYoloKernel;
        void** mAnchor;
    };

    class API RadianPluginCreator : public IPluginCreator
    {
    public:
        RadianPluginCreator();

        ~RadianPluginCreator() override = default;
    ...
    REGISTER_TENSORRT_PLUGIN(RadianPluginCreator);
};

#endif  // _RADIAN_LAYER_H
